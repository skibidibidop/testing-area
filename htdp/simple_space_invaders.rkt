...
; positive if odd
(define (rand_jump n)
  (cond[(= (modulo (random n) 2) 0) (* (random n) -1)]
       [else (random n)]))
...
(define (move gs)
  (cond[(aim? gs)
        (make-aim
         (make-posn
          (+ (posn-x (aim-ufo gs)) (rand_jump 11))
        ...
        [(fired? gs)
         (make-fired
          (make-posn
           (+ (posn-x (fired-ufo gs)) (rand_jump 11))
